...
import { BsFileEarmarkText, BsFolderFill } from 'react-icons/bs';
    ...
    >
        ...
        <Clickable file={file}>
            <div css={tw`flex-none text-neutral-400 ml-6 mr-4 text-lg pl-3`}>
                {file.isFile ? (
                    file.isSymlink ? (
                        <LuUploadCloud color={hexToRGB(medusaColors.rowFile.text, 1)} />
                    ) : file.isArchiveType() ? (
                        <RxArchive color={hexToRGB(medusaColors.rowFile.text, 1)} />
                    ) : (
                        <BsFileEarmarkText color={hexToRGB(medusaColors.rowFile.text, 1)} />
                    )
                ) : (
                    <BsFolderFill color={hexToRGB(medusaColors.rowFile.text, 1)} />
                )}
            </div>
